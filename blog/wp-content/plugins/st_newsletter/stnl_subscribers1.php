...
global $table_prefix, $wpdb, $stnl_config;
 ...
 if ( !isset($_GET['edit_st_mailinglist']) ) { ?>

<div class="wrap">

  	<h2><?php _e('Current Subscribers', 'stnl');?></h2>
		...
		/* Fetch MailingList */
			...
			$SQL = "SELECT * FROM ".$table_prefix."st_mailinglist ORDER BY st_id desc LIMIT ".$from.", ".$max_results;
		...
		<p><?php _e('No Subscribers yet', 'stnl');?></p>

		<?php } else{ ?>
        <script language="javascript">

function verify(){
	msg = "<?php _e('Are you absolutely sure that you want to delete these subscribers?','stnl');?>";
    return confirm(msg);
    }
</script>
     ...
     <label for="chk_all"><?php _e('Check/Uncheck All', 'stnl');?></label>
...
<p>
			<input type="submit" name="delete_checked" value="<?php _e('Delete All Checked', 'stnl');?>" />  <input type="submit" name="delete_all"  value="<?php _e('Delete All Subscribers', 'stnl');?>" />
</p>
			</form>
		...
		$SQL = "SELECT st_id FROM ".$table_prefix."st_mailinglist";
	...
	<input type="submit" value="Delete" onclick="return confirm('<?php _e('You are about to delete this subscriber \n  \'OK\' to delete, \'Cancel\' to stop.', 'stnl');?>')" />
...
if ( isset($_GET['edit_st_email']) ) {
	...
	$edit_st_mailinglist = $_GET['edit_st_email'];
...
?>
		...
		<?php }	?>
	    	...
	    	<p><label for="stnl_email"><?php _e('Email', 'stnl');?>: </label><br />
			   ...
			   <input name="stnl_email" id="stnl_email" type="text" value="<?php echo $e_mailinglist->st_email; ?>" /> <?php echo stripslashes($stnl_config['req']);?></p>
	...
	<select multiple="multiple" name="stnl_cats[]" id="stnl_cats" style="width:200px;" >
