...
function falbum_add_pages() {
	if (function_exists('add_options_page')) {
		...
		add_options_page('FAlbum Options', 'FAlbum', 8, basename(__FILE__), 'falbum_options_page');
...
function falbum_init() {
	...
	register_setting('falbum-options', 'falbum_options');
...
function falbum_options_page() {
	...
	if (isset ($_POST['Submit'])) {
		...
		check_admin_referer('falbum-options-options');
  ...
  <h2><?php fa_e('FAlbum Options');?></h2>
	...
	if (isset($resp)) {
		...
		if ($frob == '') {
			...
			if ($frob == '') {
				echo '<p>Error: Unable to get frob value again - Try clicking on the FAlbum options tab again. \n'.print_r($resp,true).'</p>';
			}
		}
...
?>
  ...
  <p><textarea rows="5" style="width: 98%;" name="rules"><?php echo $rewriteRule; ?>
		...
		<?php settings_fields('falbum-options'); ?>
