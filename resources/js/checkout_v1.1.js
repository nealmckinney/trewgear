...
function initCart(data) {
    ...
    setCompactMode = function ($step) {
        $step.find('form').slideUp('slow', function () {
            $step.find('.report').stop(true, true).hide().end().find('.compact').stop(true, true).slideDown('fast').end();
            $(window).trigger('resize');
        });
    },
    setReportMode = function ($step) {
        $step.find('form').slideUp('slow', function () {
            $step.find('.compact').stop(true, true).hide().end().find('.report').stop(true, true).slideDown('fast').end();
            $(window).trigger('resize');
        });
    },
    setFormEditMode = function ($step) {
        $step.find('form').find('.header').hide();
        $step.find('.report,.compact').stop(true, true).slideUp('slow', function () {
            $step.find('form').stop(true, true).slideDown('fast', function () {
    ...
    $('.review-step form').submit(function (ev) {
        ...
        emeraldcode.transact(formJSON, function (d) {
            ...
            if (d.success) {
				...
				var orderNumber = $paymentForm.find('input[name="orderNumber"]').val();
	
                $('#cart-page .right').hide();
                $('#cart-page .receipt .order-number').text(orderNumber);
				...
				var subtotal = $('#cart-page .right .sub-total').text();
				subtotal = subtotal.replace(/[^0-9.]/g, "");
				
				var state = (formJSON.shippingState) ? formJSON.shippingState : "";
				
				// AvantMetrics tracking:
				_AvantMetrics = _AvantMetrics || [];
				_AvantMetrics.push(['order',{ order_id:orderNumber, amount:subtotal, state:state, country:formJSON.shippingCountry }]);
				
				console.log("_AvantMetrics: "+_AvantMetrics);
				console.log("------------------------------------------------------");
				console.log("_AvantMetrics order:");
				console.log("------------------------------------------------------");
				console.log("order_id: "+orderNumber);
				console.log("amount: "+subtotal);
				console.log("state: "+state);
				console.log("country: "+formJSON.shippingCountry);
				console.log("------------------------------------------------------");

				// Google Ecom tracking:
				//_gaq.push(['_setAccount', 'UA-8829428-1']);
				console.log("addTrans");
				_gaq.push(['_addTrans',
				    orderNumber,           // order ID - required
				    'Trewgear',  // affiliation or store name
				    subtotal,          // total - required
				    $('#cart-page .right .tax').text(),           // tax
				    $('#cart-page .right .shipping').text(),              // shipping
				    formJSON.shippingCity,       // city
				    formJSON.shippingState,     // state or province
				    formJSON.shippingCountry             // country
				  ]);
                ...
                for (i = 0; i < emeraldcode.cartItems.length; i++) {
					...
					_gaq.push(['_addItem',
					    ...
					    item.colorName + " " + item.sizeName,   // category or variation
					    item.unitPrice,          // unit price - required
					    cart.getVal(item.sku)               // quantity - required
					  ]);
					
					_AvantMetrics.push(['item',{ order_id:orderNumber, parent_sku:item.sku, variant_sku:item.colorName + "-" + item.sizeName, price:item.unitPrice, qty:cart.getVal(item.sku) }]);
					
					console.log("------------------------------------------------------");
					console.log("_AvantMetrics item:");
					console.log("------------------------------------------------------");
					console.log("orderNumber: "+orderNumber);
					console.log("parent_sku: "+item.sku);
					console.log("variant_sku: "+(item.colorName + "-" + item.sizeName));
					console.log("price: "+item.unitPrice);
					console.log("qty: "+(cart.getVal(item.sku)));
					console.log("------------------------------------------------------");
					
					$.getScript( "/resources/js/avmws_1012281.js", function( data, textStatus, jqxhr ) {
					  console.log( data ); // Data returned
					  console.log( textStatus ); // Success
					  console.log( jqxhr.status ); // 200
					  console.log( "Load was performed: avmws_1012281." );
					});
    ...
    $('#cart-page form').dumbFormState({skipSelector:'input[name=coupon]'});
    ...
    $('#cart-page .right').dumbFixed({topSpace:-80});
...
emeraldcode.ready(function () {
    emeraldcode.getShippingOptions(function (data) {
        ...
        $('#shippingMethod').html($select.html());
...
// coupon code
$(function () {
    $('.coupon-form').submit(function (ev) {
        ev.preventDefault();
        var val = $.trim($(this).find('input[name=coupon]').val());
        if (val === '') {
            return;
        }
        emeraldcode.ready(function () {
            emeraldcode.setCoupon({ coupon: val }, function (data) {
                if (data.success) {
                    ...
                    $('.coupon-form').hide();
                    $('.coupon-form').find('input').val('');
                    $('.coupon.alert-error').show();
                }
            });
        });
    });
    $('.coupon .close').click(function (ev) {
        ev.preventDefault();
        $('.coupon-form').show();
        $('.coupon.alert').hide();
    });
